...
read m
case $m in
	y) wget https://vault.centos.org/8.4.2105/isos/x86_64/CentOS-8.4.2105-x86_64-dvd1.iso -O /tmp/CentOS-8.4.2105-x86_64-dvd1.iso ;;
	#wget http://mirrors.vinters.com/centos/8.4.2105/isos/x86_64/CentOS-8.4.2105-x86_64-dvd1.iso -O /tmp/CentOS-8.4.2105-x86_64-dvd1.iso ;;
	#wget http://mirror.bytemark.co.uk/centos/8.2009/isos/x86_64/CentOS-7-x86_64-DVD-2009.iso -O /tmp/CentOS-7-x86_64-DVD-2009.iso ;;
...
genimage centos8-x86_64-install-compute

echo
echo "Enabling repo in the image"
echo

mv /install/netboot/centos8/x86_64/compute/rootimg/etc/yum.repos.d/CentOS-Linux-BaseOS.repo /install/netboot/centos8/x86_64/compute/rootimg/etc/yum.repos.d/CentOS-Linux-BaseOS.repo.backup

cp /etc/yum.repos.d/CentOS-Linux-BaseOS.repo /install/netboot/centos8/x86_64/compute/rootimg/etc/yum.repos.d/

perl -pi -e '$line = 1 if /"CHROOT="/; print "CHROOT=/install/netboot/centos8/x86_64/compute/rootimg/\n" if !$line&&eof' /root/.bashrc;
...
yum -y --installroot=$CHROOT install ipmitool
yum -y --installroot=$CHROOT install htop
yum -y --installroot=$CHROOT install glances 
yum -y --installroot=$CHROOT install screen 
yum -y --installroot=$CHROOT install vim
...
mkdir -p /install/netboot/centos8/x86_64/compute/rootimg/root/.ssh

#chmod 700 /install/netboot/centos8/x86_64/compute/rootimg/.ssh
chmod 700 /install/netboot/centos8/x86_64/compute/rootimg/root/.ssh

cat ~/.ssh/id_rsa.pub > /install/netboot/centos8/x86_64/compute/rootimg/root/.ssh/authorized_keys

chmod 600 /install/netboot/centos8/x86_64/compute/rootimg/root/.ssh/authorized_keys
...
packimage centos8-x86_64-install-compute
...
nodels
makehosts $NODENAME;
nodeset $NODENAME osimage=centos8-x86_64-netboot-compute
EOF
...
lsdef -t osimage centos8-x86_64-netboot-compute
systemctl disable --now NetworkManager

#nodeset cn1 osimage
#chdef -t node -o cn1 netboot=yaboot
#nodeset cn12 osimage=centos8-x86_64-netboot-compute
...
#chdef -t osimage -o centos8-x86_64-netboot-compute synclists="/install/custom/netboot/compute.synclist"
#
#mkdir /install/netboot/centos8/x86_64/compute/rootimg/root/.ssh
#
#chmod 700 /install/netboot/centos8/x86_64/compute/rootimg/.ssh
#
#cat ~/.ssh/id_rsa.pub > /install/netboot/centos8/x86_64/compute/rootimg/root/.ssh/authorized_keys
#
#chmod 600 /install/netboot/centos8/x86_64/compute/rootimg/root/.ssh/authorized_key
